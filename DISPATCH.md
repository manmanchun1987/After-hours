...
## Done this hour
- **hourly 03:06 A+B+C**：patterns **split1ax** enter（擰開門／擰門入去／轉門把推／把門推開／我去擰門）＋wait（等我唾吓／唾一陣／係我唾吓／唾吓先／唔好催住先啦）＋ask_memory（功課背未／有冇溫習功課／複習功課未／背晒功課未／你溫過未呀）；guide **split1ax** 門錨 MEM_NUDGE（擰開門／等我唾吓／功課背未／複習功課未）；alex n0 stub tag；cache 仍 `?v=split1b`
- **hourly 02:03 A+B+C**：patterns **split1aw** enter（我去門度推／推開大門／門一推就入／我去拉開門）＋wait（等我唷吓／唷一陣／係我唷吓／唔好催住先）＋ask_memory（有冇複課／你複課未／背功課未／溫返功課未／功課溫返未）；guide **split1aw** 門錨 MEM_NUDGE（複課／背功課／溫返功課／等我唷吓）；alex n0 stub tag；cache 仍 `?v=split1b`
- **hourly 05:08 A+B+C**：patterns **split1at** enter（拉開門先／我行去開門／去推開門／行到門口推）＋wait（等我啱吓／啱一陣／唔好逼我／係我啱吓）＋ask_memory（溫功課／唤功課／複咋未／溫過書）；guide **split1at** 門錨 MEM_NUDGE（溫功課／唤功課／拉開門先）；alex n0 keys；cache 仍 `?v=split1b`
...
- **Status:** 已實作 strategy 池；split1ar 加厚門錨 MEM_NUDGE（唸底／溫底／背出囬）
...
#### T2 — 場景記憶接話
- **Status:** split1ax 擰開門／等我唾吓／功課背未／複習功課未；對準 recentUserLines 仍待 F
