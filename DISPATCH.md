...
## Done this hour
- **hourly 04:07 METHOD scout AD**：評核紙滑上柜；Pages+Safari；無 Imagine/無 upload/無 CF；贏 A 全景 / C 全幕 motion
...
- **Part:** Review dossier / 評核紙滑上柜
- **Use:** AD CSS `linear-gradient` paper + `translateY` slide over A review SVG
- **Never:** Imagine face · user upload · CF-only · B new empty SVG
- **Status:** METHODS row + letter 已寫；待 F/樣式 hook 接 review 節
...
- **Status:** 已實作 strategy 池；split1ar 加厚門錨 MEM_NUDGE（唸底／溫底／背出回）
